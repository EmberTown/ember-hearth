...
function spyFor(name, ctx) {
  return function spy(methodName) {
    ...
    ctx[methodName] = function () {
      ...
      let result = orig.call(this, ...arguments);

      if (methodName === 'electronCommand') {
        console.log('patching electronCommand result');
        let parts = result.split(' ');
        parts[1] = safePath(path.join(__dirname, './log-runner-ci.js'));
        result = parts.join(' ');
      }
